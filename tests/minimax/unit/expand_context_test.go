// expand_context Unit Tests - MiniMax
//
// Tests the complete expand_context flow for MiniMax without real API calls.
// Uses mock HTTP servers to simulate LLM responses.
// MiniMax uses OpenAI-compatible format, detected via X-Provider: minimax header.
package unit
...
// TestExpandContext_MiniMax_FullFlow tests the complete expand_context flow for MiniMax.
func TestExpandContext_MiniMax_FullFlow(t *testing.T) {
	...
	req.Header.Set("Authorization", "Bearer minimax-test-key")
	req.Header.Set("X-Target-URL", mockLLM.URL)
	req.Header.Set("X-Provider", "minimax") // Key: identify as MiniMax
...
// TestExpandContext_MiniMax_NoExpansionNeeded tests when MiniMax doesn't request expand.
func TestExpandContext_MiniMax_NoExpansionNeeded(t *testing.T) {
	var callCount atomic.Int32
	...
	req.Header.Set("Authorization", "Bearer minimax-test-key")
	req.Header.Set("X-Target-URL", mockLLM.URL)
	req.Header.Set("X-Provider", "minimax")
...
// TestExpandContext_MiniMax_Passthrough tests passthrough mode (no compression).
func TestExpandContext_MiniMax_Passthrough(t *testing.T) {
